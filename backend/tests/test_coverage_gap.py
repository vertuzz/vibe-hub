...
async def test_apps_filtering_and_errors(client: AsyncClient, auth_headers: dict, admin_headers: dict):
    ...
    v1_resp = await client.post("/apps/", json={"prompt_text": "V1", "tool_ids": [tool_id], "status": "Concept"}, headers=auth_headers)
    v1 = v1_resp.json()
    v2_resp = await client.post("/apps/", json={"prompt_text": "V2", "tag_ids": [tag_id], "status": "WIP"}, headers=auth_headers)
    v2 = v2_resp.json()

    # 1. Filter by tool
    resp = await client.get(f"/apps/?tool_id={tool_id}")
    assert len(resp.json()) >= 1
    assert any(v["id"] == v1["id"] for v in resp.json())

    # 2. Filter by tag
    resp = await client.get(f"/apps/?tag_id={tag_id}")
    assert len(resp.json()) >= 1
    assert any(v["id"] == v2["id"] for v in resp.json())

    # 3. Filter by status
    resp = await client.get("/apps/?status=WIP")
    assert all(v["status"] == "WIP" for v in resp.json())

    # 4. Error: Get non-existent app
    resp = await client.get("/apps/9999")
    assert resp.status_code == 404

    # 5. Error: Update non-existent app
    resp = await client.patch("/apps/9999", json={"prompt_text": "new"}, headers=auth_headers)
    assert resp.status_code == 404
    ...
    # Try to update other_app with original auth_headers
    resp = await client.patch(f"/apps/{other_app['id']}", json={"prompt_text": "hacked"}, headers=auth_headers)
    assert resp.status_code == 403

    # 7. Error: Fork non-existent app
    resp = await client.post("/apps/9999/fork", headers=auth_headers)
    assert resp.status_code == 404

    # 8. Error: Add media to others' app
    resp = await client.post(f"/apps/{other_app['id']}/media", json={"media_url": "http://evil.com"}, headers=auth_headers)
    assert resp.status_code == 403

    # 9. Success: Update own app
    resp = await client.patch(f"/apps/{v1['id']}", json={"prompt_text": "Updated V1"}, headers=auth_headers)
    assert resp.status_code == 200
    assert resp.json()["prompt_text"] == "Updated V1"

    # 10. Success: Get app
    resp = await client.get(f"/apps/{v1['id']}")
    assert resp.status_code == 200

    # 11. Success: Add media to own app
    resp = await client.post(f"/apps/{v1['id']}/media", json={"media_url": "http://myapp.com/img.png"}, headers=auth_headers)
    assert resp.status_code == 200

@pytest.mark.asyncio
async def test_likes_errors_and_unlikes(client: AsyncClient, auth_headers: dict):
    # Setup app
    app_resp = await client.post("/apps/", json={"prompt_text": "Like Me"}, headers=auth_headers)
    app = app_resp.json()
    app_id = app["id"]
    ...
    # 5. Vote on comment
    comm_resp = await client.post(f"/apps/{app_id}/comments", json={"content": "Liking this"}, headers=auth_headers)
    comm = comm_resp.json()
    comm_id = comm["id"]
    ...
    # 7. App not found for like
    assert (await client.post("/apps/9999/like", headers=auth_headers)).status_code == 404
...
async def test_notifications_read(client: AsyncClient, auth_headers: dict):
    ...
    # 2. Origin user (auth_headers) likes victim's app
    await client.post(f"/apps/{app['id']}/like", headers=auth_headers)
...
async def test_reviews_and_comments_errors(client: AsyncClient, auth_headers: dict):
    ...
    # 4. Success: Get app comments
    resp = await client.get(f"/apps/{app_id}/comments")
    assert resp.status_code == 200
    assert len(resp.json()) >= 1
    ...
    # 6. Error: Create comment on missing app
    resp = await client.post("/apps/9999/comments", json={"content": "lost"}, headers=auth_headers)
    assert resp.status_code == 404

    # 7. Error: Delete others' comment
    comm2_resp = await client.post(f"/apps/{app_id}/comments", json={"content": "Another"}, headers=auth_headers)
...
async def test_collections_edge_cases(client: AsyncClient, auth_headers: dict):
    ...
    # 1. Create collection
    col_resp = await client.post("/collections/", json={"name": "My Pack", "app_ids": [app["id"]]}, headers=auth_headers)
    col = col_resp.json()
    
    # 2. Add non-existent app to collection
    resp = await client.post(f"/collections/{col['id']}/apps/9999", headers=auth_headers)
    assert resp.status_code == 404

    # 3. Add to non-existent collection
    resp = await client.post(f"/collections/9999/apps/{app['id']}", headers=auth_headers)
    assert resp.status_code == 404
...
async def test_reviews_comprehensive(client: AsyncClient, auth_headers: dict):
    ...
    # 1. Create review
    review_resp = await client.post(f"/apps/{app_id}/reviews", json={"score": 90, "comment": "Good"}, headers=auth_headers)
    review = review_resp.json()
    
    # 2. Duplicate review
    resp = await client.post(f"/apps/{app_id}/reviews", json={"score": 80}, headers=auth_headers)
    assert resp.status_code == 400
...
async def test_follows_and_impls_edge_cases(client: AsyncClient, auth_headers: dict):
    ...
    # 5. Implementation - App not found
    assert (await client.post("/apps/9999/implementations", json={"url": "http://x.com"}, headers=auth_headers)).status_code == 404
