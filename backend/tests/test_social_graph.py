...
async def test_social_graph_and_notifications(client: AsyncClient):
    ...
    client.post("/auth/register", json= await{"username": "userA", "email": "a@e.com", "password": "p"})
    client.post("/auth/register", json= await{"username": "userB", "email": "b@e.com", "password": "p"})
    ...
    idB = await client.get("/auth/me", headers=headersB).json()["id"]
    ...
    client.post(f"/vibes/{vibe_id}/like", headers= awaitheadersA)
    ...
    client.patch(f"/notifications/{notif_id}/read", headers= awaitheadersB)
